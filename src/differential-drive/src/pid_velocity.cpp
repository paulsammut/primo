...
public:
    ...
    PidVelocity(int argc, char **argv) {
        ...
        // Paul code!!!
        // Add another publisher to the unordered map of publishers
        pub["motor_int16"] = node_handle.advertise<std_msgs::Int16>("motor_cmd_int16", 1);
    }
    ...
    void doPid() {
        ...
        // ROS_INFO("i = i + (e * dt):  %0.3f = %0.3f + (%0.3f * %0.3f)", integral, prev_integral, error, pid_dt);
        ...
        if (motor > out_max) {
            motor = out_max;
            integral = integral - (error * pid_dt);
            ROS_INFO("Check: motor > out_max");
        }
        if (motor < out_min) {
            motor = out_min;
            integral = integral - (error * pid_dt);
            ROS_INFO("Check: motor < out_min");
        }

        if ( target > 0) {  //prevent - output if target is +
            motor = std::max( 0.0, motor);
            // ROS_INFO("Check: target > 0");
        ...
        // ROS_INFO("vel:%0.3f tar:%0.3f err:%0.4f int:%0.4f der:%0.2f ## motor:%0.4f ", vel, target, error, integral, derivative, motor);
    }
    ...
    void spinOnce(ros::Rate r) {
        ...
        // Paul - adding the int16 message
        std_msgs::Int16 msgInt16;
        ...
        while (ros::ok() && ticks_since_target < timeout_ticks) {
            ...
            // Paul - casting to the int16
            short tempMotorData = static_cast<short>(motor);
            msgInt16.data = tempMotorData;
            pub["motor_int16"].publish(msgInt16);
