	...
	width_(_width), height_(_height), fps_(_fps), rgb_frame_(NULL)
{
    ...
    // Paul here:
    // Ok this is the weird shit right here. Lets figure out what the _device
    // array is, and what on earth this line is doing.
    // OK I see, it's just getting the number at the end of /dev/videoX string.
    // What a horrible fucking way of doing this. The issue is that the index
    // number is not corresponding to the right device in the DeviceInstances
    // array, which i suspect might also be causing xunit issues which is what 
    // I'm trying to fix.
